...
if openclaw cron list 2>/dev/null | grep -q "^weekly-optimization-suggestions\b"; then
    echo "  cron job 'weekly-optimization-suggestions' already registered — skipping"
else
    openclaw cron add --file cron/weekly-optimization-suggestions.json && echo "  registered cron job 'weekly-optimization-suggestions'"
fi
else
    echo "  WARNING: 'openclaw' CLI not found in PATH — register cron jobs manually:"
    echo "    openclaw cron add --file cron/weekly-optimization-suggestions.json"
fi
...
echo "  openclaw cron run --name weekly-optimization-suggestions    # trigger manually"
